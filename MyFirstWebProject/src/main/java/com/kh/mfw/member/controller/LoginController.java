...
public class LoginController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		//							ㄴ무조건 name 속성값만을 사용하진 않음
		
		// memberId / memberPw
		String userId = request.getParameter("memberId");
		String userPw = request.getParameter("memberPw");
		
		// System.out.printf("입력한 아이디 값 : %s / 입력한 비밀번호 값 : %s", userId, userPw);
		...
		MemberDTO loginMember = new MemberService().login(member); // 객체 생성(동적 할당) + 메서드 호출(객체 간의 상호 작용)
...
//		request.setAttribute("loginMember", loginMember);
		...
		// request.getRequestDispatcher("index.jsp").forward(request, response); 포워딩 방식
		
		// http://localhost/mfw 하면 웰컴파일로 그냥감
